...
public class Top10CountVisitee {


	public Top10CountVisitee() {
		// Default constructor
	}

	public static void main(String[] args) throws Exception {
		JobConf conf = new JobConf(Top10CountVisitee.class); // Create a JobConf object
		...
		//if(((RunningJob) conf).isSuccessful())
		{
			...
			org.apache.hadoop.mapreduce.Job job = new org.apache.hadoop.mapreduce.Job(confJob,"Top10Visitee");

			job.setJarByClass(Top10CountVisitee.class);
	...
	public static class Map extends MapReduceBase implements Mapper<LongWritable, Text, Text, IntWritable> {
		...
		public void map(LongWritable key, Text value,OutputCollector<Text, IntWritable> collector, Reporter reporter ) throws IOException {
			...
			if(tokens.length>19){
				names[0] = tokens[19].trim();
			}


			if(tokens.length>20){
				names[1] = tokens[20].trim();
			}



			visiteeName = names[0] + " " + names[1];
			visiteeName = visiteeName.trim();
			visiteeName = visiteeName.toUpperCase();

			Text word = new Text();
			word.set(visiteeName);
