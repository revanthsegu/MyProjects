...
public class Top10CountVisitorVisitee {


	public Top10CountVisitorVisitee() {
		// Default constructor
	}

	public static void main(String[] args) throws Exception {
		JobConf conf = new JobConf(Top10CountVisitorVisitee.class); // Create a JobConf object
		...
		//if(((RunningJob) conf).isSuccessful())
		{
			...
			org.apache.hadoop.mapreduce.Job job = new org.apache.hadoop.mapreduce.Job(confJob,"Top10VisitorVisitee");

			job.setJarByClass(Top10CountVisitorVisitee.class);
	...
	public static class Map extends MapReduceBase implements Mapper<LongWritable, Text, Text, IntWritable> {
		...
		public void map(LongWritable key, Text value,OutputCollector<Text, IntWritable> collector, Reporter reporter ) throws IOException {
			...
			if(tokens.length>0){
				names[0] = tokens[0].trim();
			}


			if(tokens.length>1){
				names[1] = tokens[1].trim();
			}


			if(tokens.length>19){
				names[2] = tokens[19].trim();
			}


			if(tokens.length>20){
				names[3] = tokens[20].trim();
			}

			visitorName = names[0] + " " + names[1] + "   "+names[2]+ " " + names[3];
