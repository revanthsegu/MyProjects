...
public class VEPOD {
	...
	public double exec(String arg,String[] evids,ArrayList<Integer> missVal) {


		VEPOD mainClass = new VEPOD();
		mainClass.readInputs(arg,evids);
		mainClass.execProg(missVal);

		return finalResult;

	}

	void execProg(ArrayList<Integer> missVals){
	...
	public void strtBucketElim(ArrayList<Integer> missVals){

		//System.out.println("Running bucket elimination");

		int remcount=0;

		for(int i=0;i<missVals.size();i++){
			//System.out.println("Removed "+ remcount+ " nodes Attempting to remove node "+missVals.get(i));
			remNode(missVals.get(i));
			remcount++;
			//printResultData();
		}
	...
	public void multiplyFuncs(ArrayList<Integer> f1,ArrayList<Double> d1,ArrayList<Integer> f2,ArrayList<Double> d2){
		...
		for(int i=0;i<totalDataSize;i++){
			...
			if(((System.currentTimeMillis() - strtTime)>20000)){
				return;
			}
	...
	void readInputs(String arg,String[] evids){
		try{

			logMax = Math.log(MAX);
			logMin = Math.log(MIN);
			//System.out.println("Looking for Network parameters file in the path: "+arg);
			...
			//System.out.println("Successfully read network properties from " + arg);

			evidCount = evids.length;
			if(evidCount>0){
				evidence = new HashMap<Integer, Integer>();

				for(int i=0;i<evidCount;i++){
					if(tokens[i].equalsIgnoreCase("0") || tokens[i].equalsIgnoreCase("1"))
						evidence.put(i, Integer.parseInt(evids[i]));
				}
