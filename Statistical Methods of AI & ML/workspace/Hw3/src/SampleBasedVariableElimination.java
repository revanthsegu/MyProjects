...
public class SampleBasedVariableElimination {
	double MAX = 10000000;
	...
	HashMap<Integer, HashSet<Integer>> graphBktVars = new HashMap<Integer, HashSet<Integer>>();
	long minOrder;
	int minOrderNode;
	ArrayList<Integer> resultFunc = new ArrayList<Integer>();
	ArrayList<Double> resultData = new ArrayList<Double>();
	ArrayList<Integer> wCutset = new ArrayList<Integer>();
	ArrayList<Integer> nonEvidVars = new ArrayList<Integer>();
	...
	public static void main(String[] args) {
		...
		SampleBasedVariableElimination mainClass = new SampleBasedVariableElimination();
		if(args.length<3){
			System.out.println("Usage: <file.uai> <w> <N>");
			System.exit(0);
		}
		...
		System.out.println("Time taken to execute the program in sec "+ (endTime - startTime)/1000);
	}
	...
	void execProg(){
		...
		wCutset = calcWCutset();
		System.out.println("W cutset is" + wCutset);
		double paraDomMul =0;


		for(int p=0;p<N;p++){

			System.out.println("Running iteration "+(p+1) +" out of "+N);

			for(int i=0; i<wCutset.size();i++){
				int val = (int)(Math.random()*paraDomainSize.get(wCutset.get(i)));
				evidence.put(wCutset.get(i), val);
				//System.out.println("Assigned val "+ val + " to "+wCutset.get(i) + " size is "+paraDomainSize.get(wCutset.get(i)) );
				if(p==0){
					paraDomMul += Math.log( paraDomainSize.get(wCutset.get(i)));
					//System.out.println("denominator is " + paraDomMul + " multiplied val is "+ paraDomainSize.get(wCutset.get(i)));
				}
			}
			strtBucketElim();

			finalSum += Math.exp(finalResLog-paraDomMul);
			System.out.println("result after elem for itr "+p +" is "+ finalSum);
		}

		//System.out.println("N is "+N + " paraDomMul is "+paraDomMul);

		finalSum = finalSum /N;
		System.out.println(  "Weight of sampling based elimination = "+  finalSum );
	...
	void readInputs(String args[]){
		try{
			...
			//System.out.println("w is "+w + " N is "+ N);
	...
	public ArrayList<Integer> calcWCutset(){

		int wMax=0;
		ArrayList<Integer> cutset = new  ArrayList<Integer>();
		HashMap<Integer, Integer> varCounts = new HashMap<Integer, Integer>();
		HashMap<Integer, Integer> bktSizes = new HashMap<Integer, Integer>();


		for(int i=0;i<totalNetPara;i++){
			if(null!=evidence && !evidence.keySet().contains(new Integer(i))){
				nonEvidVars.add(i);
			}
		}

		for(int i=0;i<totalNetPara;i++){
			System.out.println("building bucket for node "+ i);
			HashSet<Integer> varList = new HashSet<Integer>();
			for(int j=0;j<funcs.size();j++){
				//System.out.println("checking for func " +j);
				ArrayList<Integer> func = funcs.get(j);
				if(func.contains(i)){
			...
			if(varList.size()>0){
				graphBktVars.put(i, varList);
				System.out.println("building list for node "+ i+ " is "+ varList);
			}
		}

		//System.out.println("executing wcutset");

		while(true){
			bktSizes = new HashMap<Integer, Integer>();
			varCounts = new HashMap<Integer, Integer>();

			for(int i=0; i<nonEvidVars.size();i++){

				for(int j=0;j<graphBktVars.size();j++){
					//System.out.println( graphBktVars.get(j) +" "+nonEvidVars.get(i) );	

					if(null!= graphBktVars.get(j) && graphBktVars.get(j).contains(nonEvidVars.get(i))){
						if(varCounts.containsKey(nonEvidVars.get(i))){
							varCounts.put(nonEvidVars.get(i), varCounts.get(nonEvidVars.get(i))+1);
						}else{
							varCounts.put(nonEvidVars.get(i), 1);
						}


						if(bktSizes.containsKey(j)){
							bktSizes.put(j, bktSizes.get(j)+1);
						}else{
							bktSizes.put(j,1);
						}
					}
			...
			wMax=0;
			int maxCountforVar=0;
			int maxCountVar=-1;
			for(Iterator<Integer> itr = bktSizes.keySet().iterator(); itr.hasNext();){
				int key = itr.next();
				int val = bktSizes.get(key);
				if(wMax<val){
					wMax = val;
				}
			}
			...
			if(wMax > w+1){
				for(Iterator<Integer> itr = varCounts.keySet().iterator(); itr.hasNext();){
					int key = itr.next();
					int val = varCounts.get(key);
					if(maxCountforVar<val){
						maxCountforVar=val;
						maxCountVar = key;
					}
				}

				//System.out.println("maxCountVar is "+ maxCountVar);
				cutset.add(maxCountVar);
				nonEvidVars.remove(new Integer(maxCountVar));
			}else{
				//System.out.println("exiting wcutset "+ cutset);
				break;
			}
